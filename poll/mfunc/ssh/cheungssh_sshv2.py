...
import poll.mfunc.ssh.cheungssh_settings
class CheungSSH_SSH(object):
    ...
    def login(self, **kws):
        ...
        try:
            ...
            self.ip = kws["ip"]
            self.keyfile = os.path.join("/home/cheungssh/keyfile/", self.owner, kws["keyfile"])
    ...
    def get_prompt(self):
        ...
        try:
            ...
            cheungssh_info["content"] = "获取主机提示符错误:[%s]"
            cheungssh_info["status"] = False
        return cheungssh_info
    ...
    def sudo_login(self):
        ...
        try:
            ...
            cheungssh_info["status"] = False
            cheungssh_info["content"] = str(Exception)
        return cheungssh_info
